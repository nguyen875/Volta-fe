...
export const getShopProductById = async (id: string): Promise<ApiResponse<ProductDetail>> => {
  const res = await axiosInstance.get(`/shop/products/${id}`);
    ...
    const apiBase = import.meta.env.VITE_API_URL.replace("/api", '').replace("volta","Volta");
