  ...
  return (
    <Box sx={{ p: 3.5 }}>
      ...
      <Box sx={{ display: "flex", gap: 1.5, mb: 3, flexWrap: "wrap" }}>
        <StatCard
          label="Pending"
          value={countByStatus(OrderStatus.PENDING)}
          color={ADMIN_COLOR.dim}
        />
        <StatCard
          label="Paid"
          value={countByStatus(OrderStatus.PAID)}
          color={ADMIN_COLOR.orange}
        />
        <StatCard
          label="Shipped"
          value={countByStatus(OrderStatus.SHIPPED)}
          color={ADMIN_COLOR.blue}
        />
        <StatCard
          label="Delivered"
          value={countByStatus(OrderStatus.DELIVERED)}
          color={ADMIN_COLOR.green}
        />
        <StatCard
          label="Cancelled"
          value={countByStatus(OrderStatus.CANCELLED)}
          color={ADMIN_COLOR.red}
        />
      </Box>
        ...
        {(["", ...STATUS_OPTIONS] as const).map((s) => (
          <Box
            ...
            sx={{
              ...
              color:
                statusFilter === s ? ADMIN_COLOR.accentDark : ADMIN_COLOR.dim,
              border: `1px solid ${statusFilter === s ? ADMIN_COLOR.accent : ADMIN_COLOR.border}`,
              cursor: "pointer",
              "&:hover": {
                bgcolor:
                  statusFilter === s ? ADMIN_COLOR.accent : ADMIN_COLOR.muted,
              },
