...
import type {
  ...
  BundleItem,
} from "../../../../apis/bundles/bundle.interface";
...
export const BundlesTab: React.FC<{ search?: string }> = ({ search = "" }) => {
  ...
  const { data: bundleRes, isLoading } = useSWR([SWR_KEY, page, search], () =>
    getAllBundles({ search, page, limit: 50 }).then((r) => r.data),
  );

  const { data: allProducts } = useSWR("admin-products-for-bundle", () =>
  ...
  useEffect(() => {
    if (detailBundle) {
      getBundleItems(detailBundle.id).then((r) => {
  ...
  const handleAddItem = async () => {
    ...
    try {
      await addBundleItem(detailBundle.id, {
        product_id: Number(addProductId),
        quantity: 1,
      });
      const res = await getBundleItems(detailBundle.id);
      const data = res.data as BundleItem[] | { data?: BundleItem[] };
      const items = Array.isArray(data)
        ? data
        : Array.isArray(data?.data)
          ? data.data
  ...
  const getProductName = (pid: number) => {
    return (
      (allProducts ?? []).find((p: Product) => p.id === pid)?.name ?? `#${pid}`
    );
  };
  ...
  const columns: VTableColumn<Bundle>[] = [
    ...
    { key: "name", label: "Name" },
    {
      ...
      render: (row) => (
        <Typography
          sx={{ color: ADMIN_COLOR.accent, fontWeight: 700, fontSize: 13 }}
        >
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      >
        ...
        <DialogContent>
          ...
          <VTextField
            ...
            onChange={(e) =>
              setForm({ ...form, bundle_price: Number(e.target.value) })
            }
            sx={fieldSx}
          />
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <VButton variant="ghost" onClick={() => setOpen(false)}>
            Cancel
          </VButton>
          <VButton variant="secondary" onClick={handleSave} loading={saving}>
            Save
          </VButton>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <VButton variant="ghost" onClick={() => setDeleteDialogId(null)}>
            Cancel
          </VButton>
          <VButton variant="danger" onClick={handleDelete}>
            Delete
          </VButton>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <VButton variant="ghost" onClick={() => setDetailBundle(null)}>
            Close
          </VButton>
        </DialogActions>
      </Dialog>
