...
import { Box, Typography, Modal, Divider, IconButton } from "@mui/material";
...
import type {
  Discount,
  RequestCreateDiscountDto,
} from "../../../../apis/discounts/discount.interface";
...
const schema = Yup.object({
  ...
  min_order: Yup.number()
    .min(0, "Min order must be ≥ 0")
    .required("Min order is required"),
  uses_remaining: Yup.number()
    .nullable()
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      await onSubmit({
        ...
        uses_remaining:
          values.uses_remaining !== null ? Number(values.uses_remaining) : null,
        ...
        <form onSubmit={formik.handleSubmit}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <VTextField
              ...
              onChange={(v) =>
                formik.setFieldValue("code", String(v ?? "").toUpperCase())
              }
            ...
            >
              ...
              <VTextField
                fieldType="number"
                label={
                  formik.values.type === DiscountType.Percent
                    ? "Value (%)"
                    : "Value ($)"
                }
                required
            ...
            >
              <VTextField
                ...
                error={
                  formik.touched.min_order
                    ? (formik.errors.min_order as string)
                    : undefined
                }
                sx={darkFieldSx}
              />
              <VTextField
                ...
                onChange={(v) =>
                  formik.setFieldValue("uses_remaining", v === "" ? null : v)
                }
                sx={darkFieldSx}
              />
            </Box>
            ...
            <VTextField
              ...
              onChange={(v) =>
                formik.setFieldValue("expires_at", v === "" ? null : v)
              }
              sx={darkFieldSx}
            />
...
export const DiscountsTab: React.FC = () => {
  ...
  const { data: discountsResp, mutate } = useSWR<Discount[]>("discounts", () =>
    getAllDiscounts().then(
      (r) => (r.data as unknown as { data: Discount[] }).data ?? r.data,
    ),
  );
  ...
  const columns: VTableColumn<Discount>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <Typography
          sx={{ fontFamily: "monospace", fontSize: 13, color: ADMIN_COLOR.dim }}
        >
      ...
      render: (r) => (
        <Typography
          sx={{
            fontSize: 13,
            color:
              r.uses_remaining !== null && r.uses_remaining < 100
                ? ADMIN_COLOR.red
                : ADMIN_COLOR.dim,
          }}
        >
    ...
    },
    {
      ...
      render: (r) => (
        <Typography sx={{ fontSize: 12, color: ADMIN_COLOR.dim }}>
          {r.expires_at
            ? dayjs(r.expires_at).format("MMM D, YYYY")
            : "No expiry"}
        </Typography>
      ),
    },
